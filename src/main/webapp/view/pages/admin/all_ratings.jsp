...
<head>
    ...
    <title><fmt:message key="title.all_ratings"/></title>
</head>
<body>
<header class="header">
    <div class="container">
        <div class="header__inner">
            <nav class="header__nav">
                <ul class="header__menu">
                    ...
                    <li class="header__menu-item">
                        <a style="margin-top: 20px; color: gray;" class="header__menu-link" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_RATINGS}">
                    ...
                    <li class="dropdown">
                        ...
                        <div class="dropdown-content">
                            <c:choose>
                                <c:when test="${sessionScope.localization.equals('en')}">
                                    <a style="font-size: 40px" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_RATINGS}&lang=ru">Рус</a>
                                </c:when>

                                <c:otherwise>
                                    <a style="font-size: 40px" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_RATINGS}&lang=en">Eng</a>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </li>
...
<main class="main">
    <div class="info">
        <div style="min-height: 600px;" class="container">
            ...
            <br/> <br/>
            <table id="all_entities_ratings" class="sortable">
                <tr>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_PK_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_FK_USER_ID_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_FK_IMAGE_ID_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_RATING_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_REVIEW_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_STATUS_COLUMN}"/></th>
                    <th><fmt:message key="table.ratings.column.${DataBaseInfo.RATINGS_TABLE_CREATED_AT_COLUMN}"/></th>
                </tr>
                <c:forEach items="${requestScope.all_ratings}" var="rating">
                    <tr>
                        <td>${rating.getId()}</td>
                        <td>${rating.getUserId()}</td>
                        <td>${rating.getImageId()}</td>
                        <td>${rating.getRating()}</td>
                        <td>${rating.getReview()}</td>
                        <td><fmt:message key="status.rating.${rating.getStatus().toString().toLowerCase()}"/></td>
