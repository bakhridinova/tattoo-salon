...
<head>
    ...
    <title><fmt:message key="title.all_images"/></title>
</head>
<body>
<header class="header">
    <div class="container">
        <div class="header__inner">
            <nav class="header__nav">
                <ul class="header__menu">
                    ...
                    <li class="header__menu-item">
                        <a style="margin-top: 20px; color:gray;" class="header__menu-link" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_IMAGES}">
                    ...
                    <li class="dropdown">
                        ...
                        <div class="dropdown-content">
                            <c:choose>
                                <c:when test="${sessionScope.localization.equals('en')}">
                                    <a style="font-size: 40px" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_IMAGES}&lang=ru">Рус</a>
                                </c:when>

                                <c:otherwise>
                                    <a style="font-size: 40px" href="${pageContext.request.contextPath}/home?command=${CommandType.ALL_IMAGES}&lang=en">Eng</a>
                                </c:otherwise>
                            </c:choose>
                        </div>
                    </li>
...
<main class="main">
    <div class="info">
        <div style="min-height: 600px;" class="container">
            ...
            <br/> <br/>
            <table id="all_entities_images" class="sortable">
                <tr>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_PK_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_FK_USER_ID_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_STATUS_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_SHORT_DESCRIPTION_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_lONG_DESCRIPTION_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_ORDERS_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_RATING_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_PRICE_COLUMN}"/></th>
                    <th><fmt:message key="table.images.column.${DataBaseInfo.IMAGES_TABLE_URL_COLUMN}"/></th>
                </tr>
                <c:forEach items="${requestScope.all_images}" var="image">
                    <tr>
                        <td>${image.getId()}</td>
                        <td>${image.getUserId()}</td>
                        <td><fmt:message key="status.image.${image.getStatus().toString().toLowerCase()}"/></td>
                        <td>${image.getShortDescription()}</td>
                        <td>${image.getLongDescription()}</td>
                        <td>${image.getOrders()}</td>
                        <td>${image.getRating()}</td>
                        <td>${image.getPrice()}</td>
                        <td><a href="${image.getUrl()}">#</a></td>
                    </tr>
                </c:forEach>
            </table>
